...
#     # predictions = remove_negatives(dets, meta, num)
...
def remove_negatives(detections, class_names, num):
    ...
    predictions = []
    for j in range(num):
        for idx in range(class_names.classes):
            if detections[j].prob[idx] > 0:
                bbox = detections[j].bbox
                bbox = (bbox.x, bbox.y, bbox.w, bbox.h)
                predictions.append((class_names.names[idx], detections[j].prob[idx], (bbox)))
    return predictions
...
def detect(network, class_names, image, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    num = pnum[0]
    if nms:
        do_nms_sort(detections, num, class_names.classes, nms)
...
network_predict = lib.network_predict#_ptr
network_predict.argtypes = [c_void_p, POINTER(c_float)]
