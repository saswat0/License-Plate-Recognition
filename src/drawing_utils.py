...
def write2img(Img,label,strg,txt_color=(0,0,0),bg_color=(0,0,255),font_size=1):
	wh_img = np.array(Img.shape[1::-1])
	Img_PIL = Image.fromarray(cv2.cvtColor(Img, cv2.COLOR_BGR2RGB))
	font = cv2.FONT_HERSHEY_SIMPLEX
	Font = ImageFont.truetype('/usr/share/fonts/wqy-microhei/wqy-microhei.ttc',60)
	wh_text,v = cv2.getTextSize(strg, font, font_size, 3)
	bl_corner = label.tl()*wh_img
	#strg = strg.encode('utf8')
	...
	ccy = (bl_corner[1]-tl_corner[1])/2+tl_corner[1]
	ccx = (br_corner[0]-bl_corner[0])/2+bl_corner[0]
	...
	draw.text((ccx,ccy),strg,bg_color,font=Font)
